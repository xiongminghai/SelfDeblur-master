...
parser.add_argument('--data_path', type=str, default="datasets/NIND_blur", help='path to blurry image')
parser.add_argument('--save_path', type=str, default="results/NIND_blur/", help='path to deblurring results')
parser.add_argument('--clean_path', type=str, default="datasets/NIND", help='path to deblurring results')
...
files_source.sort()
clean_files_source = glob.glob(os.path.join(opt.clean_path, '*.png'))
clean_files_source.sort()
...
# start #image
for f,clean_f in zip(files_source,clean_files_source):
    INPUT = 'noise'
    pad = 'reflection'
    LR = 0.01
    ...
    pi=IQALosspi().type(dtype)
    niqe=IQALossniqe().type(dtype)
    nrqm=IQALossnrqm().type(dtype)
    cnnniqa=IQALosscnniqa().type(dtype)
    clipiqa=IQALossclipiqa().type(dtype)
    ...
    file = open(os.path.join(opt.save_path, "%s_logs.txt"%(imgname)), 'w')
    file.truncate(0)
    ...
    for step in tqdm(range(num_iter)):
        ...
        if step%10==0:
            out_x1,out_x2=pair_downsampler(out_x)
            loss_mse = mse(out_x1, out_x2)
            out_x1_np = torch_to_np(out_x1)
            out_x1_np = out_x1_np.squeeze()
            out_x2_np = torch_to_np(out_x2)
            out_x2_np = out_x2_np.squeeze()
            Q1, SX, SY = omqdi_q1(out_x1_np, out_x2_np)
            loss_niqe=niqe(out_x)
            loss_pi=pi(out_x)
            loss_nrqm=nrqm(out_x)
            loss_clipiqa=clipiqa(out_x)
            out_x_cal_np = torch_to_np(out_x)
            out_x_cal_np = out_x_cal_np.squeeze()
            Q1_t, SX_t, SY_t = omqdi_q1(out_x_cal_np, out_x_cal_np)
            cropw, croph = padw, padh
            out_x_cal_np = out_x_cal_np[cropw // 2:cropw // 2 + img_size[1], croph // 2:croph // 2 + img_size[2]]
            out_x_cal_np = np.uint8(255 * out_x_cal_np)
            out_x_cal_np = cv2.merge([out_x_cal_np, cr, cb])
            out_x_cal_np = cv2.cvtColor(out_x_cal_np, cv2.COLOR_YCrCb2BGR)
            psnr=compare_psnr(out_x_cal_np.astype(np.float64)/255.,img_tar.astype(np.float64)/255.)
            file.write('step %d   psnr %f   loss_mse %f    loss_niqe %f   loss_pi: %f   loss_nrqm: %f   loss_clipiqa: %f   Q1: %f    SX: %f    SY: %f    Q1_t: %f    SX_t: %f    SY_t: %f\n' % (step,psnr,loss_mse.item(),loss_niqe.item(),loss_pi.item(),loss_nrqm.item(),loss_clipiqa.item(),Q1,SX,SY,Q1_t,SX_t,SY_t))
        ...
        if (step + 1) % opt.save_frequency == 0:
            ...
            save_path = os.path.join(opt.save_path, '%s_x_%d.png' % (imgname,step+1))
            ...
            save_path = os.path.join(opt.save_path, '%s_k_%d.png' % (imgname,step+1))
